...
export const StatsView: React.FC<StatsViewProps> = ({ bookings, expenses }) => {
    ...
    return (
        <div className="max-w-5xl mx-auto p-4 sm:p-6 w-full pb-20 md:pb-6">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-6">Estadísticas y Crecimiento</h2>

            {/* Top Cards */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm transition-colors duration-200">
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium uppercase mb-1">Crecimiento Mensual</p>
                    <div className="flex items-end gap-2">
                        <span className="text-3xl font-bold text-gray-900 dark:text-white">{Math.abs(growth).toFixed(1)}%</span>
                        {growth > 0 ? (
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm transition-colors duration-200">
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium uppercase mb-1">Beneficio Anual</p>
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm transition-colors duration-200">
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium uppercase mb-1">Promedio Mensual</p>
            ...
            {/* Charts */}
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm overflow-x-auto transition-colors duration-200">
                <h3 className="font-bold text-gray-700 dark:text-gray-200 mb-6">Evolución Anual (Ingresos vs Gastos)</h3>
                    ...
                    {monthlyData.map((data, idx) => {
                        ...
                        return (
                            <div key={idx} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                                ...
                                <span className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-2 font-medium rotate-0 whitespace-nowrap capitalize">
                                    {data.month.split(' ')[0]}
                                </span>
                            </div>
                        );
